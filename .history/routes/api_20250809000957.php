...
Route::post('/ably/auth', function (Request $request) {
    // For the standalone user app, we'll pass the user ID in the request
    // For the admin, we are using static user ID 1
    $userId = $request->input('user_id', 1); // Default to admin (1) if not provided
    ...
    $capabilities = $isAdmin
        ...
        : ["chat:user_{$userId}" => ['publish', 'subscribe']]; // User can only access their channel
