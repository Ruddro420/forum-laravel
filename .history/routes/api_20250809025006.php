...
Route::post('/ably/auth', function(Request $request) {
    ...
    $capabilities = ['chat:user_'.$userId => ['publish','subscribe']];

    if ($user->is_admin) {
        // Admin can access all chat channels
        $capabilities = ['chat:*' => ['publish','subscribe']];
    }
...
// Fetch all messages between current user and admin (id=1)
Route::get('/chat/messages/{userId}', function($userId) {
    $currentUserId = (int) request()->input('current_user_id');

    // Validate users exist
    if (!User::find($userId) || !User::find($currentUserId)) {
        return response()->json(['error' => 'User not found'], 404);
    }
...
// Send message and publish to Ably
